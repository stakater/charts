...
| **Edge p95 latency** | The customer-facing latency promise (Area 1 is the only layer with a customer SLA). One number, alert-threshold colored — if it's green, latency complaints are probably not the platform edge. Long-running watches are excluded; including them would pin this to a constant 60s (see the alerts guide). |
...
| **etcd members healthy** | The `KcpEtcdMembersDown` input: healthy etcd members vs cluster size. A dead member produces ABSENT metrics — this count is the only place it shows (proven live: a member with a read-only PVC was invisible for 3 days). Below 3 = failure tolerance reduced or gone. |
...
| **Edge latency** | Quantiles fail differently: a p95 shift is systemic slowdown (etcd, shard saturation), a p99-only shift is a heavy tenant or pathological request shape. **Long-running watches are excluded** — they sit in the `+Inf` bucket with no label to filter on, and including them pins both quantiles to the top finite bucket permanently (see the alerts guide). The readable view of edge speed; the alert keys off the slow-ratio panel, not these lines. |
...
| **Edge slow-request ratio (watches excluded)** | The SLI behind `KcpFrontProxyLatencyHigh`, and the reason that alert is a ratio rather than a p95 in seconds. Share of non-watch requests over the latency budget, measured inside the sub-60s population so the open-watch count cannot skew it. Exists so a firing latency alert has somewhere to land — without it an operator sees a healthy p95 and no explanation. Gaps mean an idle edge (`0/0 = NaN`), which correctly does not alert. |
